...
from app.schemas.user import UserCreate, UserLogin, UserOut, UserProfileUpdate, UserRegister
...
class AuthService:
    async def register(
        ...
        *,
        ...
        if exists_user:
            raise HTTPException(status_code=400, detail=f"用户名 {obj_in.username} 已存在")
    ...
    async def update_profile(
        ...
        *,
        ...
        cleaned_update_data = {key: ("" if value is None else value) for key, value in update_data.items()}
        update_obj = UserProfileUpdate(**cleaned_update_data)

        updated_obj = await crud_user.update(db=db, db_obj=db_obj, obj_in=update_obj)
